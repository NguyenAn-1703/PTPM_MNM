...
export const Sidebar: React.FC<SidebarProps> = ({ status, isLoading, documentCount, messages, chatSessions, activeSessionId, onClearVectorStore, onClearHistory, onNewChat, onSelectSession, onDeleteSession, isMobileOpen, onCloseMobile }) => {
    ...
    return (
        <aside
            className={`fixed inset-y-0 left-0 z-40 flex w-[290px] flex-col border-r border-slate-200/60 bg-white/92 shadow-2xl shadow-slate-900/10 backdrop-blur-xl transition-transform duration-300 dark:border-slate-700/70 dark:bg-slate-950/88 lg:static lg:z-20 lg:w-[300px] lg:translate-x-0 ${
                ...
                <div className="mb-4 space-y-1">
                    {orderedSessions.map((session, idx) => {
                        ...
                        return (
                            ...
                            >
                                <span
                                    className={`inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[10px] font-bold ${isCurrent ? "bg-sky-200 text-sky-700 dark:bg-sky-500/30 dark:text-sky-100" : "bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-200"}`}
                                >
                                    {idx + 1}
                                </span>
