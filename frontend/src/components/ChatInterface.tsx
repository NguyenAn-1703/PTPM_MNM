...
const SUGGESTIONS = [
  ...
  'Liệt kê các ý quan trọng theo dạng bullet',
  'Cho tôi ví dụ minh họa từ tài liệu',
  'So sánh 2 phần nội dung nổi bật',
];
      ...
      <footer className="z-20 px-3 pb-5 pt-3 md:px-8 md:pb-7">
        <div className="mx-auto w-full max-w-4xl">
          <form onSubmit={handleSubmit} className="chat-shell rounded-[28px] p-2 shadow-xl shadow-slate-900/10">
            <div className="rounded-3xl bg-white/88 px-2 py-1 dark:bg-slate-950/75">
              <div className="flex items-end gap-2">
                ...
                >
                  <span className="material-icons-round" style={{ fontSize: '17px' }}>attach_file</span>
                </button>
                <textarea
                  ref={textareaRef}
                  value={input}
                ...
                <button
                  ...
                  title="Gửi"
                >
                  <span className="material-icons-round" style={{ fontSize: '18px' }}>north</span>
                </button>
              </div>
          ...
          <p className="mt-3 text-center text-xs text-slate-500 dark:text-slate-400">
            AI có thể trả lời chưa chính xác. Hãy kiểm tra lại các thông tin quan trọng.
          </p>
        </div>
      </footer>
    </div>
...
const WelcomeMessage: React.FC<{ onPickSuggestion: (text: string) => void }> = ({ onPickSuggestion }) => (
  <div className="mx-auto flex h-full w-full max-w-4xl flex-col items-center justify-center px-3 py-14 text-center animate-fade-in-up">
    <div className="mb-6 flex h-18 w-18 animate-soft-pulse items-center justify-center rounded-3xl border border-sky-200 bg-white shadow-lg shadow-sky-500/15 dark:border-sky-500/30 dark:bg-slate-900">
      <span className="material-icons-round text-sky-500" style={{ fontSize: '34px' }}>auto_awesome</span>
    </div>
    ...
    <p className="mt-3 max-w-xl text-sm leading-relaxed text-slate-500 dark:text-slate-400 md:text-base">
      Đặt câu hỏi về tài liệu đã upload và nhận câu trả lời có trích dẫn nguồn tham chiếu.
    </p>
      ...
      <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
        {!isUser && (
          <div className="mt-1 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-2xl border border-sky-200 bg-white dark:border-slate-700 dark:bg-slate-900">
            <span className="material-icons-round text-sky-500" style={{ fontSize: '18px' }}>smart_toy</span>
          </div>
        )}
        ...
        <div className={`max-w-[92%] md:max-w-[82%] ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
          <div
            className={
              `rounded-3xl px-4 py-3 text-[15px] leading-relaxed shadow-sm ${
                isUser
                  ? 'bg-slate-900 text-white dark:bg-sky-500 dark:text-slate-950'
                  : 'border border-slate-200/80 bg-white text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100'
              }`
            }
          >
          ...
          {!isUser && message.contexts && message.contexts.length > 0 && (
            <div className="mt-2 w-full">
              ...
              >
                <span className="material-icons-round" style={{ fontSize: '13px' }}>menu_book</span>
                {message.contexts.length} nguồn tham chiếu
                <span className={`material-icons-round transition-transform ${expandedContext === message.id ? 'rotate-180' : ''}`} style={{ fontSize: '14px' }}>expand_more</span>
              </button>
                    ...
                    <div key={idx} className="rounded-2xl border border-slate-200/80 bg-white p-3 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
                      <div className="mb-1.5 flex items-center justify-between gap-2 text-[11px]">
                        <div className="flex items-center gap-1 text-slate-500 dark:text-slate-400">
                          <span className="material-icons-round text-sky-500" style={{ fontSize: '12px' }}>description</span>
                          <span className="truncate">{ctx.metadata.filename || 'Unknown Document'}</span>
                        </div>
                        <span className="rounded-md bg-slate-100 px-1.5 py-0.5 font-mono text-slate-500 dark:bg-slate-800 dark:text-slate-300">
                          {(1 - ctx.score).toFixed(2)}
                        </span>
                      </div>
